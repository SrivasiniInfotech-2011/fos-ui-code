...
import {FOSCCApiEndPoints, FOSCCMicroApiDomain} from "../../../../core/common/literals";
...
export class FOSRequestService{
  ...
  fetchRequestInfo(requestId:number, callback: Function): void{
    let endPoint = this.utilsService.buildApiEndpoint(FOSCCMicroApiDomain.REQUEST,FOSCCApiEndPoints.REQUEST_API);
    if(endPoint.trim()){
  ...
  fetchInterestedParties(requestId:number, callback: Function):void{
    let endPoint = this.utilsService.buildApiEndpoint(FOSCCMicroApiDomain.REQUEST,FOSCCApiEndPoints.REQUEST_INTERESTED_PARTY_API);
    if(endPoint.trim()){
      //this.translate.instant('services.configuration'),this.translate.instant('services.errorLoading'); -- Todo - Need to check this
    }
